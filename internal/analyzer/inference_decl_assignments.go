...
func inferAssignExpression(ctx *InferenceContext, n *ast.AssignExpression, table *symbols.SymbolTable, inferFn func(ast.Node, *symbols.SymbolTable) (typesystem.Type, typesystem.Subst, error), typeMap map[ast.Node]typesystem.Type) (typesystem.Type, typesystem.Subst, error) {
	...
	if n.AnnotatedType != nil {
		...
		// Check for implicit conversion (e.g. Int -> Float)
		valType = injectImplicitConversion(n, explicitType, valType)
	...
	if ident, ok := n.Left.(*ast.Identifier); ok {
		...
		if sym, defScope, ok := table.FindWithScope(ident.Value); ok {
			...
			if sym.IsPending {
				...
				table.DefineWithNode(ident.Value, declaredType, ctx.CurrentModuleName, ident)
				if ctx.ResolutionMap != nil {
					if sym, found := table.Find(ident.Value); found {
			...
			if sym.Type != nil {
				// Apply current substitutions to existing variable type to ensure we check against refined type
				currentSymType := sym.Type.Apply(ctx.GlobalSubst).Apply(totalSubst)
				subst, err := typesystem.UnifyAllowExtraWithResolver(currentSymType, valType, table)
				if err != nil {
					return nil, nil, inferErrorf(n, "cannot assign %s to variable %s of type %s", valType, ident.Value, currentSymType)
				}

				// Check for implicit conversion (e.g. Int -> Float)
				valType = injectImplicitConversion(n, currentSymType, valType)
			...
			table.DefineWithNode(ident.Value, declaredType, "", ident)
			if ctx.ResolutionMap != nil {
				if sym, found := table.Find(ident.Value); found {
					ctx.ResolutionMap[ident] = sym
				}
...
func inferFunctionLiteral(ctx *InferenceContext, n *ast.FunctionLiteral, table *symbols.SymbolTable, inferFn func(ast.Node, *symbols.SymbolTable) (typesystem.Type, typesystem.Subst, error)) (typesystem.Type, typesystem.Subst, error) {
	...
	for i, p := range n.Parameters {
		...
		enclosedTable.DefineWithNode(p.Name.Value, localType, "", p.Name)

		// Map parameter AST node to its symbol in ResolutionMap
		if ctx.ResolutionMap != nil {
			sym, found := enclosedTable.Find(p.Name.Value)
			if found {
				ctx.ResolutionMap[p.Name] = sym
			}
		}
	}
...
// injectImplicitConversion checks if we are assigning Int to Float, and if so,
// injects a runtime cast (intToFloat call) into the AST.
// Returns the updated valType (which becomes Float if cast happened).
func injectImplicitConversion(n *ast.AssignExpression, targetType, valType typesystem.Type) typesystem.Type {
	target := typesystem.UnwrapUnderlying(targetType)
	val := typesystem.UnwrapUnderlying(valType)

	if tTarget, ok := target.(typesystem.TCon); ok && tTarget.Name == "Float" {
		if tVal, ok := val.(typesystem.TCon); ok && tVal.Name == "Int" {
			// Inject intToFloat(val)
			n.Value = &ast.CallExpression{
				Function: &ast.Identifier{
					Value: "intToFloat",
				},
				Arguments: []ast.Expression{n.Value},
			}
			// Return Float as the new type of the expression
			return targetType
		}
	}
	return valType
